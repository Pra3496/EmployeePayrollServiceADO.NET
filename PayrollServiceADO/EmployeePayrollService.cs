 	...
 	public void GetSelectedDataFromDataBase(string Name)
        {
            ...
            try
            {
                List<PayrollServiceModel> addressBookList = new List<PayrollServiceModel>();

                using (sqlConnection)
                {
                    sqlConnection.Open();
                    SqlCommand sqlCommand = new SqlCommand("spGetSelectedDataFromDataBase", sqlConnection);
                    ...
                    if (sqlReader.HasRows)
                    {
                        while (sqlReader.Read())
                        {
                            ...
                            addressBookList.Add(payrollServiceModel);
                        }
                        foreach (PayrollServiceModel item in addressBookList)
                        {
                            Console.WriteLine("+++++++++++[ {0} ]+++++++++++\n", ++iCnt);
